...
myEmail="tyler dot francis at jelec.com"
...
## Also, I totally copied my previous cleanup script, disk2vhd_cleanup.sh which needed to be designed to cleanup multiple repos.
...
if [ $usedSpace -ge $fillLine ]; then
	...
	cat $cleanDir/clean.mail | mail -s "The VM backup repo was getting full, so I delete the Production Party backups from these dates." $myEmail
fi
